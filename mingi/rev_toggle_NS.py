...
class MAG240M(LightningDataModule):
    ...
    def convert_batch_train(self, batch_size, n_id, adjs):
        ...
        return Batch(x=x, y=y, adjs_t=[adj_t for adj_t, _ in adjs])
...
class RGNN(LightningModule):
    def __init__(self, model: str, in_channels: int, out_channels: int,
        ...
        if self.model == 'rgat':
            for _ in range(num_layers):
                self.convs.append(
                    ModuleList([
                        GATConv(in_channels, in_channels // heads,
        ...
        for _ in range(num_layers):
            self.norms.append(BatchNorm1d(in_channels))

        for _ in range(num_layers):
            self.norms.append(BatchNorm1d(hidden_channels))

        for _ in range(num_layers):
            self.skips.append(Linear(in_channels, in_channels))
    ...
    def forward(self, x: Tensor, adjs_t: List[SparseTensor]) -> Tensor:
        hf = len(adjs_t)//2
        #x should be x[:adj_t.size(1)]
        x_target = x[:adjs_t[0].size(1)]
        for i in range(0, hf):
            adj_t = adjs_t[i]
            x_in = x_target
            x_target = x[:adj_t.size(0)]
            out = self.skips[i](x_target)
            #print(i,x.shape,x_target.shape,x_in.shape,self.convs[i])
            for j in range(self.num_relations):
                ...
                if subadj_t.nnz() > 0:
                    out += self.convs[i][j]((x_in, x_target), subadj_t)

            x_target = self.norms[i](out)
            x_target = F.elu(x_target) if self.model == 'rgat' else F.relu(x_target)
            x_target = F.dropout(x_target, p=self.dropout, training=self.training)
        
        x = x_target

        for i in range(hf, hf*2):
            adj_t = adjs_t[i]
            x_target = x[:adj_t.size(0)]
            out = self.skips[i](x_target)
            #print(i,adj_t.size(0),adj_t.size(1),x.shape,x_target.shape)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hidden_channels', type=int, default=1025)
    ...
    parser.add_argument('--bit', type=int, default=14) # Used in ensemble step.
    ...
    if args.debug:
        ...
        name=f'/toggle-NS_mingi_rev_p={args.label_disturb_p}_batch={args.batch_size}'
    else:
        name=f'/toggle-NS_mingi_rev_p={args.label_disturb_p}_batch={args.batch_size}_hidden={args.hidden_channels}'
