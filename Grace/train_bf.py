...
class GRACE(LightningModule):
    ...
    def training_step(self, batch, batch_idx: int):
        ...
        if(batch_idx%100==0):
            print('train_loss : '+str(train_loss.item())+' | loss : '+str(train_loss)+' | time : '+str(time.time()-t0)+" | batch : "+str(batch_idx))
            f_log.write('train_loss : '+str(train_loss.item())+' | loss : '+str(train_loss)+' | time : '+str(time.time()-t0)+" | batch : "+str(batch_idx))
            f_log.write('\n')
...
if __name__ == '__main__':
    ...
    if not args.evaluate:
        ...
        model = GRACE(args.model, datamodule.num_features+153,
                    datamodule.num_classes, args.hidden_channels, args.hidden_channels,
